...
const textRadiansPerSecond = 0.12;
const textAngleStep = 0.09;
const hoverInset = 12;
...
export function ContourImage({ src, label, className }: ContourImageProps) {
  ...
  useEffect(() => {
    ...
    const draw = (now = 0) => {
      ...
      const textAngle = reduceMotion.matches ? 0 : ((now - startTime) * textRadiansPerSecond) / 1000;
      previousTime = now;
      ...
      // Clip img to blob shape using CSS polygon (percentages = resolution-independent)
      if (imgRef.current) {
        const polygon = path
          .slice(0, pointCount)
      ...
      // --- Canvas: blob fill (fades out as photo fades in) + orbiting text (always visible) ---
      context.clearRect(0, 0, width, height);

      // Blob fill fades as image reveals
      ...
      // Orbiting text — always at full opacity
      context.fillStyle = colorString({
        red: Math.max(0, grayValue - 60),
        green: Math.max(0, grayValue - 60),
        blue: Math.max(0, grayValue - 60),
      });
      context.font = `300 10px "Cormorant Garamond", ${fontFamily || 'serif'}`;
      context.textAlign = 'center';
      context.textBaseline = 'middle';

      const angleOffset = seed * 0.01 + textAngle;

      for (let i = 0; i < Math.ceil(fullTurn / textAngleStep); i += 1) {
        const character = contourText[i % contourText.length];
        const angle = angleOffset + i * textAngleStep;
        const nextAngle = angle + 0.01;
        const cosine = Math.cos(angle);
        const sine = Math.sin(angle);
        const nextCosine = Math.cos(nextAngle);
        const nextSine = Math.sin(nextAngle);
        const targetWobble =
          1 +
          Math.sin(angle * 5 + time * 0.32 + phase) * 0.012 +
          Math.sin(angle * 3 - time * 0.21 + phase) * 0.01;
        const nextTargetWobble =
          1 +
          Math.sin(nextAngle * 5 + time * 0.32 + phase) * 0.012 +
          Math.sin(nextAngle * 3 - time * 0.21 + phase) * 0.01;
        const baseX = centerX + cosine * radiusAt(angle, time, baseRadius, seed, phase) * xRadius * xMotion;
        const baseY = centerY + sine * radiusAt(angle, time, baseRadius, seed, phase) * yRadius * yMotion;
        const nextBaseX = centerX + nextCosine * radiusAt(nextAngle, time, baseRadius, seed, phase) * xRadius * xMotion;
        const nextBaseY = centerY + nextSine * radiusAt(nextAngle, time, baseRadius, seed, phase) * yRadius * yMotion;
        const targetX =
          width / 2 + Math.sign(cosine) * (width / 2 - hoverInset) * Math.abs(cosine) ** 0.5 * targetWobble;
        const targetY =
          height / 2 + Math.sign(sine) * (height / 2 - hoverInset) * Math.abs(sine) ** 0.5 * targetWobble;
        const nextTargetX =
          width / 2 + Math.sign(nextCosine) * (width / 2 - hoverInset) * Math.abs(nextCosine) ** 0.5 * nextTargetWobble;
        const nextTargetY =
          height / 2 + Math.sign(nextSine) * (height / 2 - hoverInset) * Math.abs(nextSine) ** 0.5 * nextTargetWobble;
        const x = baseX + (targetX - baseX) * easedReveal;
        const y = baseY + (targetY - baseY) * easedReveal;
        const nextX = nextBaseX + (nextTargetX - nextBaseX) * easedReveal;
        const nextY = nextBaseY + (nextTargetY - nextBaseY) * easedReveal;
        const normalLength = Math.hypot(x - hoverCenterX, y - hoverCenterY) || 1;

        context.save();
        context.translate(
          x + ((x - hoverCenterX) / normalLength) * 10,
          y + ((y - hoverCenterY) / normalLength) * 10,
        );
        context.rotate(Math.atan2(nextY - y, nextX - x));
  ...
  return (
    <div className={`relative aspect-square w-full min-w-0 cursor-pointer ${className ?? ''}`}>
      {/* Photo behind canvas — clipped to blob shape via polygon() updated each frame */}
      <img
        ref={imgRef}
        src={src}
        alt=""
        ...
        style={{ opacity: 0 }}
      />
      {/* Canvas: animated blob fill + orbiting text */}
      <canvas
        ref={canvasRef}
